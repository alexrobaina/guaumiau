...
async function main() {
  ...
  for (const providerData of PROVIDERS_DATA) {
    ...
    // Create user with provider profile and services
    const user = await prisma.user.create({
      data: {
        ...
        roles: providerData.roles as any,
        isActive: true,
        ...
        serviceProvider: {
          create: {
            ...
            services: {
              create: providerData.services as any,
            },
          } as any,
        },
      },
    ...
    console.log(`  - Profile ID: ${(user as any).serviceProvider?.id}`);
    console.log(`  - Services: ${(user as any).serviceProvider?.services.length}`);
  }
